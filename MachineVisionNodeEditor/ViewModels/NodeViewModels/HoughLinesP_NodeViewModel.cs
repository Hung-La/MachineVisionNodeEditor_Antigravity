using MachineVisionNodeEditor.Commands;
using MachineVisionNodeEditor.Models.NodeModels;
using MachineVisionNodeEditor.Models.NodeOperationModels;
using MachineVisionNodeEditor.Models.NodePropertyModels;
using System.Windows.Input;

namespace MachineVisionNodeEditor.ViewModels.NodeViewModels
{
    public class HoughLinesP_NodeViewModel : NodeControl_NodeViewModel<HoughLinesP_NodeModel, HoughLinesP_NodePropertyModel, HoughLinesP_NodeOperationModel>
    {
        public override ICommand ShowImageCommand { get; protected set; }

        public HoughLinesP_NodeViewModel() : base()
        {
            Initialize();
            EnsureInitialPorts();
        }

        public HoughLinesP_NodeViewModel(HoughLinesP_NodeModel model) : base(model)
        {
            Initialize();
            EnsureInitialPorts();
        }

        public HoughLinesP_NodeViewModel(NodeModel nodeModel) : base(nodeModel is HoughLinesP_NodeModel vm ? vm : new HoughLinesP_NodeModel
        {
            X = nodeModel.X,
            Y = nodeModel.Y,
            Type = NodeType.HoughLinesP
        })
        {
            Initialize();
            EnsureInitialPorts();
        }

        private void Initialize()
        {
            NodeModel.Title = "Hough Lines P";
            ShowImageCommand = new RelayCommand(
                () => NodePropertyModel?.Context.OutputImage != null && !NodePropertyModel.Context.OutputImage.IsDisposed && !NodePropertyModel.Context.OutputImage.Empty(),
                () => ShowNodeImages());
        }

        private void EnsureInitialPorts()
        {
            if (NodeModel.InputPorts.Count == 0) NodeModel.AddPort(PortType.Input);
            while (NodeModel.OutputPorts.Count < 2)
            {
                NodeModel.AddPort(PortType.Output);
            }
        }

        protected override HoughLinesP_NodeOperationModel CreateOperationModel() => new HoughLinesP_NodeOperationModel();
    }
}
